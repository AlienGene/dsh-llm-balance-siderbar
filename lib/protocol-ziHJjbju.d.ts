...
type Level = 'normal' | 'warn' | 'error';
/** Which sources the corner card shows. */
type DisplayMode = 'current' | 'summary' | 'all';
 ...
 */
type ColorKey = Level | 'active';
/** A qualifier the card localizes next to a balance. */
type BalanceNote = 'deficit' | 'unavailable';
/**
 ...
 */
interface BalanceThresholdRule {
...
type BalanceThresholds = Readonly<Record<string, BalanceThresholdRule>>;
/**
 ...
 */
type QuotaWindowKind = 'rolling' | 'week' | 'month' | 'day';
/**
...
type UnsupportedReason = 'no-usage-api';
...
type ErrorCode = 'NO_CREDENTIAL' | 'UNAUTHORIZED' | 'TOKEN_EXPIRED' | 'RATE_LIMITED' | 'TIMEOUT' | 'NETWORK' | 'HTTP_ERROR' | 'BAD_RESPONSE' | 'NOT_SUPPORTED';
...
interface SessionSelectionLike {
  provider?: string;
  model?: string;
}
...
//#endregion
export { SourceState as _, ColorKey as a, TranslateFn as b, Level as c, QuotaWindow as d, QuotaWindowKind as f, SourceSnapshot as g, SessionSummaryLike as h, BalanceThresholds as i, ModelSelectionProjectionLike as l, SessionSelectionLike as m, BalanceNote as n, DisplayMode as o, SessionListStateLike as p, BalanceThresholdRule as r, ErrorCode as s, BalanceEntry as t, PercentMode as u, StateResponse as v, UnsupportedReason as x, Thresholds as y };